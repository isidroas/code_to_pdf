...
def disk_usage(path):
    ...
    size, name = (
        subprocess.run(["du", "-bs", str(path)], capture_output=True)
        .stdout.decode()
        .split()
    )
...
from fnmatch import fnmatch  # noqa: E402,F401
...
def sort_key(path):
    return (
        ...
        # path.is_dir(),
        disk_usage(path)
        if not path.name == "heap_test.stdout"
        else disk_usage(path.parent / "main.c") + 1,
        # NegateCmp(path.name),
        path.name,
    )
